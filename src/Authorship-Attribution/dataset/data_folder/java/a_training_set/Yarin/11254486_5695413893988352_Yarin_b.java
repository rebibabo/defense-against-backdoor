 ...
 public class B {
     ...
     private void solve(String a, String b, int pos, long an, long bn, long p) {
         ...
         if (!wilda && !wildb) {
             ...
             solve(a, b, pos+1, an, bn,p/10); 
             solve(a, b, pos+1, an+p, bn,p/10); 
             solve(a, b, pos+1, an, bn+p,p/10); 
         } else if (wilda) {
             solve(a, b, pos+1, an+bc*p, bn+bc*p,p/10); 
             if (bc > 0) solve(a, b, pos+1, an+(bc-1)*p, bn+bc*p,p/10); 
             if (bc < 9) solve(a, b, pos+1, an+(bc+1)*p, bn+bc*p,p/10); 
         } else if (wildb) {
             solve(a, b, pos+1, an+ac*p, bn+ac*p,p/10); 
             if (ac > 0) solve(a, b, pos+1, an+ac*p, bn+(ac-1)*p,p/10); 
             if (ac < 9) solve(a, b, pos+1, an+ac*p, bn+(ac+1)*p,p/10); 
         }
