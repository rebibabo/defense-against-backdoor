from style_change_method import pointer_to_array
from style_change_method import array_to_pointer
...
def transform_random(program_style, program_name, save_path, converted_styles):
    if program_style['5.1'] == 0 and program_style['5.2'] == 0:
        return
    if program_style['5.1'] > program_style['5.2']:
        converted_styles.append('5')
        array_to_pointer.program_transform_save_div(program_name, save_path)
    else:
        converted_styles.append('5')
        pointer_to_array.program_transform_save_div(program_name, save_path)
