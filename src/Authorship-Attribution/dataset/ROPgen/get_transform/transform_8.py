from style_change_method import init_declaration, var_init_split
...
def get_instances_and_styles(e, root):
    ...
    for instance in style1:
        path_list = [root.getpath(instance)]
...
def transform_random(program_style, program_name, save_path, converted_styles):
    if program_style['8.1'] == 0 and program_style['8.2'] == 0:
        return
    if program_style['8.1'] > program_style['8.2']:
        converted_styles.append('8')
        var_init_split.program_transform_save_div(program_name, save_path)
    else:
        converted_styles.append('8')
        init_declaration.program_transform_save_div(program_name, save_path)
