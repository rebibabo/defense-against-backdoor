...
def split_func(xml_file, c_file, func_name, line_start, line_end, new_file):
    ...
    func = get_function(e, func_name)  # Find function
    vars = get_vars_and_types(func)
    vars.update(get_tmp_vars_and_types(func))
    charge_vars_list = []  # Store the value of the variable that needs to be changed
    vars_pos = []
    vars_lines = set()
    new_vars = {}
    ignore_vars = []
        ...
        if str(curr_line) not in vars_lines:
            ...
            for item in vars_pos:
                ...
                if line == curr_line:
                    ...
                    line_pos = pos_end + 1
                    new_func += name.encode('utf8') + b'.val'  # Modify the variables in the function
                    f.read(len(name))
    ...
    for var in charge_vars_list:
        charge_var(var[0], var[1],
                   e)  # The first parameter is the variable type, the second parameter is the variable name
    ...
    for child in children:
        ...
        if tag.localname in nested_elems:
            if tag.localname == 'if_stmt':
                ...
                if len(child.xpath('src:block/src:block_content', namespaces=ns)) > 0:
