        ...
        if define[0][0].tag != '{http://www.srcML.org/srcML/src}struct' and define[0][
            ...
            for name in names:
                ...
                if name.text == define_name and name.getparent().tag != '{http://www.srcML.org/srcML/src}typedef':
                    ...
                    if decl_stmt is not None and decl_stmt.tag == '{http://www.srcML.org/srcML/src}decl_stmt' and len(
                            decl_stmt) != 1:
                        decl_stmt[0].tail = ';\n'
...
def get_style(xml_path):
    ...
    for define in defines:
        if len(define) <= 1: continue
        if len(define[0][0]) == 0 and define[0][0].tag != '{http://www.srcML.org/srcML/src}struct' and define[0][
            0].tag != '{http://www.srcML.org/srcML/src}union' and define[0][
            0].tag != '{http://www.srcML.org/srcML/src}enum':
            define_name = define[1].text
            name_replace = ''
