...
from style_change_method import c_lib_to_cpp
...
# transform_code C input redirections (by calling freopen) in 'func' to C++ ifstream
...
# transform_code C output redirections (by calling freopen) in 'func' to C++ ofstream
# return True if there is at least one freopen call in 'func', False otherwise
...
# transform_code C++ ifstream in 'func' to C input redirections (by calling freopen)
...
# transform_code C++ ofstream in 'func' to C output redirections (by calling freopen)
...
# transform_code C++ ofstream in 'func' to C++ cout plus freopen to redirect it,
...
def transform_fout_to_cout(func, tree_root, ignore_list=[], max_modifications=999):
    ...
    if not ofs_names: return flag, new_ignore_list
...
def transform_fin_to_cin(func, tree_root, ignore_list=[], max_modifications=999):
    ...
    if not ifs_names: return flag, new_ignore_list
    ...
    for cout_stmt in cout_stmts:
        ...
        for child in children[2:]:
            ...
            if tag.localname == 'literal':
                ...
                # don't transform_code this cout statement
                if fmt_spec == '':
                    should_skip = True
                    continue
                fmt_str += fmt_spec
...
# if we transform_code cin/cout into scanf/printf, include <stdio.h> if it's not already included
...
def program_transform_save_div(program_name, save_path, style1='13.1', style2='13.2', max_modifications=999):
