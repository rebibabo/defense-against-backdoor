    ...
    for item in for_stmts:
        for for_stmt in item:
            ...
            if for_prev_path in ignore_list:
                continue
            # 判断for是否为独立语句，如果是，则需要加'{}'
            if for_stmt.getparent() is not None:
                for_stmt_parent = for_stmt.getparent()
                tag1 = etree.QName(for_stmt_parent)
                if tag1.localname == 'block_content':
                    if for_stmt_parent.getparent() is not None:
                        tag2 = etree.QName(for_stmt_parent.getparent())
                        if tag2.localname == 'block' and for_stmt_parent.getparent().text is None:
                            for_stmt_parent.getparent().text = '{'

                        if for_stmt_parent.tail is None:
                            for_stmt_parent.tail = '}'
...
if __name__ == '__main__':
    program_path = '"/home/yjy/code/2023/CARROT/data1/origin/1/1018.c"'
    program_transform(program_path)
