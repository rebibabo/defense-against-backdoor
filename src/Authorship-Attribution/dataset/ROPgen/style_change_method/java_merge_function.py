...
from style_change_method.var_name_style import transform
...
def merge_func(parser, src_func, dst_func):
    # main function cannot be merged
    ...
    # Different return value types cannot be combined
    ...
    # Replace all original function call statements
    ...
    # print(src_func_args)  #Obtain the parameter name: the type is represented by a dictionary
    dst_func_args = get_args_and_types(dst_func)
    # print(dst_func_args)  #Obtain the parameter name: the type is represented by a dictionary
    ...
    # Check if there are different types of parameters with the same name
    ...
    for arg_name in shorter_list:
        if arg_name in longer_list:
            if longer_list[arg_name] != shorter_list[arg_name]:
                # print('Argument conflict! Cannot merge!')    #Same name but different type
                return False
...
def transform_by_line_cnt(src_author, dst_author, save_to='tmp.xml'):
    ...
    dst_avg_func_len = count_func_avg_len_by_author(
        dst_author)  # Record the average length of the target author function
    ...
    for file in file_list:
        ...
        for func in funcs:
            ...
            other_funcs.append((func, func_len))  # Store function node and function length
            if func_len < dst_avg_func_len:
                short_funcs.append((func,
                                    func_len))  # Function nodes and distances shorter than the average function length of the target author
