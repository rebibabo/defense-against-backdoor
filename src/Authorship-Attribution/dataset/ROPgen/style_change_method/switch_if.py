...
doc = None
flag_switch = True  # judge whether switch can be transform_code
...
# default structure transform_code
...
# case structure transform_code
def transform_case(case_elems, var):
    # transform_code the first case
    case_first = case_elems[0]
    case_first.tag = 'if'
    case_first.text = 'if'
    transform_case_content(case_first, var)
    # transform_code the case after the first one
...
def trans_tree(e):
    ...
    for switch_elem in switch_elems:
        ...
        # get default
        default_elem = get_default(switch_elem)
        # default sentence starts to transform_code
        transform_default(default_elem)
        # every case starts to transform_code
