...
def is_camel_case(name):    #myCamel
...
def underscore_to_camel(name):  #this_is->thisIs
    if not is_underscore(name): return name
    new_name = ''
    is_prev_underscore = False
    for ch in name:
...
def camel_to_underscore(name):  #myCamel->my_camel
...
def initcap_to_underscore(name):    #MyCamel->my_camel
...
def program_transform_save_div(program_name, save_path, src_style, dst_style):
    e = init_parser(os.path.join(save_path, program_name + '.xml'))
    transform(e, src_style, dst_style, [], None)
