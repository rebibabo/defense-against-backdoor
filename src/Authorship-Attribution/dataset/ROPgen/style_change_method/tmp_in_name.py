...
def alter_for_name(for_elems, author_for_names, typedef_names, func_s):
    ...
    for author_name in author_for_names:
        ...
        author_name_elem = author_name_elems[0] if len(author_name_elems) > 0 else (
            author_name.xpath('src:init//src:argument_list//src:name', namespaces=ns)[0] if len(
                author_name.xpath('src:init//src:argument_list//src:name', namespaces=ns)) > 0 else None)
    ...
    for for_elem in for_elems:
        ...
        if later_name in typedef_names:
            continue
        #
        class_name = func_s.xpath('./src:template/src:parameter_list/src:parameter/src:name', namespaces=ns)
...
def get_author_for_names(dst_author):
    ...
    for file_elem in file_list:
        ...
        file_elem = os.path.join(dst_author if os.path.isdir(dst_author) else '', file_elem)
