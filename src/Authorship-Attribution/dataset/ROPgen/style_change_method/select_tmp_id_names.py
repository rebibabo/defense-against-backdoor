...
def is_incr_decr(name_elem):
    cond1 = name_elem.xpath(
        'following-sibling::src:operator[text()="++" or text()="--"] | preceding-sibling::src:operator[text()="++" or text()="--"]',
        namespaces=ns)
...
def get_template_names(func):
    vars = {}
    vars_type = {}
    templates = func.xpath('.//src:template/src:parameter_list/src:parameter', namespaces=ns) + func.xpath(
        './/src:typedef', namespaces=ns) + \
                func.xpath('.//src:struct//src:decl', namespaces=ns) + func.xpath('.//src:union//src:decl',
                                                                                  namespaces=ns)
...
def replace_names(src_author_file, src_vars_cnt, src_vars_info, dst_vars_cnt, dst_vars_info, is_tmp, save_to,
                  ignore_list=[], keep_log=False):
    i = 0
    is_first = True
    broken = False
    new_ignore_list = []
    ...
    for src_name, src_cnt in src_vars_cnt:
        ...
        whitelist += __key_words__
        whitelist += __ops__
        whitelist += __macros__
        whitelist += __special_ids__
        ...
        try:
            ...
            backup = os.path.join('/'.join(save_to.split('/')[:-1]), 'transform_code.bak')
            if not is_first:
        ...
        for name in names:
            ...
            if is_tmp and tmp_vars.get(name.text, None) is None: continue
        ...
        file = save_to
        backup = os.path.join('/'.join(file.split('/')[:-1]), 'transform_code.bak')
...
# transform_code temporary identifier names
...
def transform_tmp_id_names(src_author, dst_author, ignore_list=[], save_to='tmp.xml', keep_log=False):
    ...
    # print(intersect)
    # print(dst_vars_info)
    diff = {k: dst_vars_cnt[k] for k in set(dst_vars_cnt) - intersect - set(src_templates) - set(
        dst_templates)}  # exclude identifier names that source author already uses
    dst_vars_info = {k: dst_vars_info[k] for k in set(dst_vars_info) - intersect - set(src_templates) - set(
        dst_templates)}  # exclude identifier names that source author already uses
    ...
    for src_filename in file_list:
        ...
        new_ignore_list, this_broken, doc, var_replace_log = replace_names(src_file, src_vars_cnt, src_vars_info, diff,
                                                                           dst_vars_info, True, save_to, ignore_list,
                                                                           keep_log)
...
def is_transformable(src_author, dst_author):
    ...
    src_all, src_all_vars_info = get_vars_cnt_by_author(src_author, tmp_only=False)
    diff = {k: dst_vars_cnt[k] for k in set(dst_vars_cnt) - set(src_vars_cnt) - set(src_all)}
...
def program_transform(path_program, path_author):
    transform_tmp_id_names(path_program, path_author, ignore_list=[], save_to='./style/transform_code.xml')
