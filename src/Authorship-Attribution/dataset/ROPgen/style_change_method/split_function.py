...
from style_change_method.static_dyn_mem import static_to_dyn
...
def identity_copy_charwise(f, newbuf, col_start, col_end=None):
    if col_end:
        for _ in range(col_start, col_end + 1):
            ch = f.read(1)
            newbuf += ch
    elif col_end != 0 and col_end is not None:
        crlf = [10, 13]
        next_char = f.read(1)
...
def check_continue_break(func, line_start, line_end):
    continue_breaks = func.xpath('.//src:continue', namespaces=ns) + func.xpath('.//src:break',
                                                                                namespaces=ns) + func.xpath(
        './/src:goto', namespaces=ns)
...
# do the splitting
# argument '_xml_file' is the srcML XML path
...
def split_func(xml_file, c_file, func_name, line_start, line_end, new_file):
    ...
    for k, v in new_vars.items():
        ...
        if '[' not in v:
            ...
            new_func += (v.split('[')[0].encode('utf8') + b' ' + k.encode('utf8') + b'ary[' + '['.join(
                v.split('[')[1:]).encode('utf8'))
            new_func_call += k
    new_func += b') {\n'
    new_func_call += ');\n'
        ...
        for func in funcs:
            ...
            for elem in elems:
                ...
                if elem_len_sum >= len_diff:
                    start_elem = elem
                    break
            if start_elem is None: continue
            flag = True
            ...
            if not check_return(func, start_elem_line_start, end_elem_line_end):
                new_func_path = split_func(filename, c_file, func_name, start_elem_line_start, end_elem_line_end,
                                           'new.c')
                shutil.rmtree('new.c')
