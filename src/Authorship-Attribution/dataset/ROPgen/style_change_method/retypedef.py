...
key_words = ['int', 'long', 'union', 'enum', 'long long', 'double', 'char', 'string', 'float', 'vector', 'pair', 'bool',
             'void', 'signed', 'unsigned', 'short', 'std']
...
def creat_def_list(e, src_var_names):
    ...
    for define in defines:
        if len(define) <= 1: continue
        if define[0][0].tag != '{http://www.srcML.org/srcML/src}struct' and define[0][
            0].tag != '{http://www.srcML.org/srcML/src}union' and define[0][
            0].tag != '{http://www.srcML.org/srcML/src}enum':
            for l in ls:
                if define[1].text == l[1]:
                    l[0] += 1
                    flag = 1
...
def del_typedef(del_ls, e, node_typ):
    ...
    for type_define in type_defines:
        ...
        if type_define[0][0].tag != '{http://www.srcML.org/srcML/src}struct' and type_define[0][
            0].tag != '{http://www.srcML.org/srcML/src}union' and type_define[0][
            0].tag != '{http://www.srcML.org/srcML/src}enum':  # len(define[0][0]) == 0 and
